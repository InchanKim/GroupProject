...
Our team conducted an anaylysis to determine how an LSU graduate's past internships affect their starting salary after graduation, if at all. We focused on LSU students graduating with a Bachelor's Degree, as their salaries should be vastly different from the salaries of Master's Degree and PhD students.
...
We first compared the starting salaries for those accepted to full time positions after graduation against 3 different indicators:
...
```{r}
# Instead of copy pasting the code from the R script, you can source it
# source("StudentInternships.R") should work
# You can just call the objects that you want (e.g. graphs) where you need them
