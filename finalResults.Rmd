...
There were `r count(noInterns)` students who did not have internships.
...
xxx %>% 
  ggplot() +
  geom_bar(aes(interns, fill = interns)) -> xGraph
xGraph
...
internCurrent <- inner_join(internship, currPosition, by = c('graduationId')) %>%
...
byMajor <- left_join(internCurrent, major, by = 'graduationId')%>%
  ...
  mutate(nOrs = if_else(condition = intOrganizationId == organizationId, true = "Same Company", false = "New Company")) %>% 
  select(graduationId, internshipId, intOrganizationId, organizationId, nOrs, positionType, majorName, titleCode, idCurrentPosition, academicCredit) %>%
  filter(!is.na(nOrs))
...
ftstay <- left_join(stayed, fulltime, by = "idCurrentPosition") %>% 
  select(salary) %>% filter(salary > 1)
ftnew <- left_join(newco, fulltime, by = "idCurrentPosition") %>% 
  select(salary) %>% filter(salary > 6)
